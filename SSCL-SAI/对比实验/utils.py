...
class plot_report():
    ...
    def draw_report(self, draw, form='step', start2end=[], skip=1, epoch_id=[], report_folder='../nats_results',
                    figure_size=(6, 6), xlim=None, ylim=None, save_folder=None, dpi = 1000, title_fontsize = 15,
                    labelsize = 15):
        ...
        for key in draw:
            ...
            #plt.legend([key], fontsize = labelsize)
            ...
            if form == 'epoch':
                ...
                if key == "loss_without_reg":
                    name = "Contrastive Loss"
                elif key == "loss_reg":
                    name = "Orthogonal Regularization Loss"
                else:
                    name = key
                ...
                if not save_folder is None:
                    ...
                    fl_ = key + '-epoch:' + str(self.epoch_id[0]) + str(self.epoch_id[-1]) + '.jpg'
                    save_file = os.path.join(
                        save_folder, fl_)
                    plt.savefig(save_file, format='jpg', dpi = dpi)
                ...
                plt.xlabel('epoch-step(k)', fontsize = labelsize)
                if key == "loss_without_reg":
                    name = "Contrastive Loss"
                elif key == "loss_reg":
                    name = "Orthogonal Regularization Loss"
                ...
                if not save_folder is None:
                    if not os.path.exists(save_folder):
                        os.mkdir(save_folder)
                    save_file = os.path.join(
                        save_folder, '{}-step-{}to{}.jpg'.format(
                            key, x[0], x[-1]))
                    plt.savefig(save_file, format='jpg', dpi = dpi)
...
class read_top_sen():
    ...
    def create_excel(self, threhold):
        if len(self.record) == 0:
            self.load_record()
        if not os.path.exists('../excel'):
            os.mkdir('../excel')

        folder = '../excel/category_(threhold-{})'.format(threhold)
        if not os.path.exists(folder):
            os.mkdir(folder)
    ...
    def find_two_categories(self, threhold = 0.4):
        assert threhold < 0.5 and threhold > 0.0
        if len(self.record) == 0:
            self.load_record()
        if not os.path.exists('../two_categories'):
            os.mkdir('../two_categories')

        book = openpyxl.Workbook()
        sheet = book.create_sheet(title='threhold = {}'.format(threhold), index=0)
        columns = ['id', '处理后的文本', '原文本', 'asp_weight', 'attn_weight', 'pre_aspect', 'pre_asp_confidence', 'label']
        sheet.append(columns)

        for i, asp_weight in enumerate(self.record['asp_weight']):
            asp_w = np.asarray(asp_weight)
            a, b = np.argsort(asp_w)[::-1][:2]
            if asp_weight[a] > threhold and asp_weight[b] > threhold:
                content = [str(i)]
                for key in columns:
                    if key == 'id':
                        continue
                    if key == 'pre_aspect':
                        content.append(str([a, b]))
                    else:
                        content.append(str(self.record[key][i]))
                sheet.append(content)
        book.save('../two_categories/threhold = {}.xlsx'.format(threhold))
        book.close()
