...
class modelSSCL(modelSSCLBase):
    def __init__(self,args):
        ##args:
        # 层相关:emb_size; device; smooth_factor; distance
        # 模式:  task
    ...
    def build_encoder(self, input_, mask_):
        ...
        #分配注意力
        ctx_ = attn_.unsqueeze(1) @ emb_                   #(batch_size, 1, seq_len)
        ctx_ = ctx_.squeeze(1)
    ...
    def build_pipe(self):
        ...
        if self.args.task[-5:] == 'train' :
            ...
            score_neg_arr = []
            for itm in self.batch_data['neg_examples']:                 #错误（疑）：根据论文的公式，这里多了一次循环
